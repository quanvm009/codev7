...
class report_stock_move(osv.osv):
    ...
    _columns = {
        ...
        'sale_order_id': fields.many2one('sale.order', 'SO Number', readonly=True),
        'sale_line_id': fields.many2one('sale.order.line', 'SO Number', readonly=True),
                ...
                SELECT
                        ...
                        sum(
                            (CASE WHEN sp.type in ('out') THEN
                                  ...
                                  ELSE  (sm.product_qty * pu.factor / pu2.factor) 
                            END)
                        ) as product_qty,
                        sum(
                            (CASE WHEN sp.type in ('out') THEN
...
#
# class report_stock_inventory_by_size(osv.osv):
#    _name = "report.stock.inventory.by.size"
#    _auto = False
#    _columns = {
...
#        'size_id':fields.many2one('product.size', 'Size', readonly=True),
#        'sale_order_id':fields.many2one('sale.order', 'PO Number', readonly=True),
#    }
#    def init(self, cr):
#        tools.drop_view_if_exists(cr, 'report_stock_inventory_by_size')
#        cr.execute("""
#                CREATE OR REPLACE view report_stock_inventory_by_size AS (
#                    (SELECT
...
#                        m.location_id,  m.location_dest_id,m.size_id, m.sale_id,
...
#                        m.product_id as product_id, pt.categ_id as product_categ_id, l.usage as location_type, l.scrap_location as scrap_location,
#                        m.company_id,
#                        m.state as state, m.prodlot_id as prodlot_id,m.size_id, m.sale_id as sale_order_id,
#                        coalesce(sum(pt.standard_price * m.product_qty * pu.factor / pu2.factor)::decimal, 0.0) as value,
#                        coalesce(sum(m.product_qty * pu.factor / pu2.factor)::decimal, 0.0) as product_qty
#                    FROM
#                        stock_move m
...
#                            WHERE m.state != 'cancel'
#                    GROUP BY
#                        m.id, m.product_id, m.product_uom, pt.categ_id, m.partner_id, m.location_id, m.location_dest_id,m.size_id, m.sale_id,
#                        m.prodlot_id, m.date, m.state, l.usage, l.scrap_location, m.company_id, pt.uom_id, to_char(m.date, 'YYYY'), to_char(m.date, 'MM')
#                    )
#                );
#                        """)
# report_stock_inventory_by_size()
